...
- **`Dockerfile`** — vendored from `moddevices/mod-plugin-builder`. Builds
  ...
  targeting `glibc 2.27` + `gcc 9.4.0`. Tag: `moddwarf-cross:latest`.
- **`build-plugin.sh`** — runs *inside* the container. Does a native
...
so it is shared by every plugin built from the same template. That's why
the tag is the plugin-independent `moddwarf-cross` and not `sitar-cross`:
...
- **Plugin loads but won't drag onto the pedalboard**: check that
  ...
  `build-plugin.sh` should have populated them via the top-level
